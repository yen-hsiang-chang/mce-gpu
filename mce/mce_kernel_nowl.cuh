...
__global__ void mce_kernel_l1_ip_nowl(
  GLOBAL_HANDLE<T> gh,
...
template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 2048 / BLOCK_DIM_X)
__global__ void mce_kernel_l1_ipx_nowl(
  GLOBAL_HANDLE<T> gh,
  queue_callee(queue, tickets, head, tail))
{
   LOCAL_HANDLE<T> lh;
...
template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 2048 / BLOCK_DIM_X)
__global__ void mce_kernel_l2_ip_nowl(
  GLOBAL_HANDLE<T> gh,
  queue_callee(queue, tickets, head, tail))
{
  LOCAL_HANDLE<T> lh;
...
template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 2048 / BLOCK_DIM_X)
__global__ void mce_kernel_l2_ipx_nowl(
  GLOBAL_HANDLE<T> gh,
  queue_callee(queue, tickets, head, tail))
{
  LOCAL_HANDLE<T> lh;
