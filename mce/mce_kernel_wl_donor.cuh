  ...
  queue_callee(queue, tickets, head, tail))
{
  ...
  if (threadIdx.x == 0)
  {
    ...
    printf("%u, %u, %llu\n", blockIdx.x, __mysmid(), donations);
  }
}

template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 2048 / BLOCK_DIM_X)
__global__ void mce_kernel_l1_ipx_wl_donor(
  GLOBAL_HANDLE<T> gh,
  queue_callee(queue, tickets, head, tail))
{
  LOCAL_HANDLE<T> lh;
...
template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 2048 / BLOCK_DIM_X)
__global__ void mce_kernel_l2_ip_wl_donor(
  GLOBAL_HANDLE<T> gh,
  queue_callee(queue, tickets, head, tail))
{
  LOCAL_HANDLE<T> lh;
...
template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 2048 / BLOCK_DIM_X)
__global__ void mce_kernel_l2_ipx_wl_donor(
  GLOBAL_HANDLE<T> gh,
  queue_callee(queue, tickets, head, tail))
{
  LOCAL_HANDLE<T> lh;
